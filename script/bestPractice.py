...
# ------------------------------------------------------------
BATCHSIZE = 1
INPUT_SHAPE = [BATCHSIZE, 3, 1024, 1024]
DEVICE = "cuda"
...
with torch.no_grad():
    ...
    # embed_size = model.image_embedding_size
    # mask_input_size = [4 * x for x in embed_size]
    dummy_inputs = {
        "image": torch.randn(1, 3, 1024, 1024, dtype=torch.float).to(DEVICE),
    }
    ...
    with warnings.catch_warnings():
        ...
        # with open("out/onnx.model", "wb") as f:
        torch.onnx.export(
            onnx_model,
    ...
    with ENABLE_CUDA_KERNEL():
        ...
        #     platform=PLATFORM,
        #     setting=QS,
        #     onnx_export_file="out/onnx.model",
        #     device=DEVICE,
        #     verbose=0,
        # )
        quantized = quantize_onnx_model(
            onnx_import_file="out/onnx.model",
            calib_dataloader=CALIBRATION,
            calib_steps=32,
            do_quantize=True,
            input_shape=None,
            ...
            inputs=torch.rand(INPUT_SHAPE).to("cuda"),
        )
    ...
    # ------------------------------------------------------------
    ONNX_OUTPUT_PATH = "out_q/model.onnx"
        ...
        for reference_output, onnxruntime_output in zip(
            ...
            y_pred.append(
                convert_any_to_torch_tensor(
                    reference_output[oidx], device="cpu"
                ).unsqueeze(0)
            )
            y_real.append(
                convert_any_to_torch_tensor(
