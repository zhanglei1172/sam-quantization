...
class MyTVMQuantizer(TensorRTQuantizer):
    ...
    def init_quantize_config(self, operation: Operation) -> OperationQuantizationConfig:
        ...
        for x, in_c in zip(operation.inputs, OQC.input_quantization_config):
            if (
                x is None
                or (not x.is_parameter)
                or in_c.state == QuantizationStates.FP32
                or in_c.quant_min == -128
            ):
                continue
            else:
                in_c.quant_max = 127
                in_c.quant_min = -128
                in_c.policy = QuantizationPolicy(
                    QuantizationProperty.SYMMETRICAL
                    + QuantizationProperty.LINEAR
                    + QuantizationProperty.PER_TENSOR
                )
                in_c.observer_algorithm = "minmax"
...
with torch.no_grad():
    ...
    with ENABLE_CUDA_KERNEL():
        ...
        #         )
        #     if op.name.endswith("attn/proj/MatMul"):
        #         QS.dispatching_table.append(
        #             operation=op.name, platform=TargetPlatform.FP32
        #         )
        ...
        for op in search_engine.opset_matching(
            sp_expr=lambda x: x.name.endswith("/attn/Softmax"),
            rp_expr=lambda x, y: not x.name.endswith("attn/MatMul"),
            ep_expr=lambda x: x.name.endswith("attn/Split"),
            direction="up",
        ):
            # if op.type != "LayerNormalization":
            QS.dispatching_table.append(operation=op.name, platform=TargetPlatform.FP32)
        ...
        reports = sorted(reports.items(), key=lambda x: x[1], reverse=True)
        int_op = []
        for name, report in reports:
            int_op.append(quantized.operations[name].outputs[0].name)
        torch.cuda.empty_cache()
        # variable_analyse(
        #     quantized,
        #     dataloader=calibration_dataloader,
        #     interested_outputs=int_op,
        #     collate_fn=collate_fn,
        #     dequantize=True,
        #     running_device="cpu"
        # )
