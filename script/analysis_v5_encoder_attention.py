...
def attention_forward(op: Operation, values: List[torch.Tensor], ctx: None, **kwargs):
    ASSERT_NUM_OF_INPUT(op=op, values=values, min_num_of_input=4, max_num_of_input=4)
    values = VALUE_TO_EXECUTING_DEVICE(op=op, ctx=ctx, values=values)

    q, k, v, pos = values[0], values[1], values[2], values[3]

    scale = op.attributes.get("scale", 1.0)

    B, C, H, K = q.shape
    atten = q @ k.transpose(-1, -2) * scale
    atten += pos.reshape(B, C, H, H)
...
register_operation_handler(
    ...
    operation_type="SAMEncoderTritonFlashAttention",
    platform=PLATFORM,
)
register_operation_handler(
    handler=attention_forward,
...
class MyMerger(GraphMerger):
    def fuse_selfattention_encoder(self):
        ...
        for softmax, (scale, pos1, pos2) in softmaxs.items():
            ...
            inputs = [x for x in split.outputs]
            assert len(inputs) == 3
            add2 = softmax.inputs[0].source_op.inputs[0].source_op
            var2 = add2.inputs.pop(1)

            add1 = add2.inputs[0].source_op
            var1 = add1.inputs.pop(1)

            var1.dest_ops.clear()
            var2.dest_ops.clear()

            pos = self.graph.create_operation(
                op_type="Add",
                attributes={},
                inputs=[var1, var2],
                outputs=[self.graph.create_variable()],
            )
            ...
            while torm_ops:
                ...
                if op != split:
                    for x in op.inputs:
                        if (
                            x.source_op
                            and len(x.source_op.outputs) == 1
                            and len(x.source_op.outputs[0].dest_ops) == 1
                        ):
                            torm_ops.add(x.source_op)
                    self.graph.remove_operation(op, remove_unlinked_variable=True)

            op = self.graph.create_operation(
                op_type="SAMEncoderTritonFlashAttention",
                attributes={
                    "scale": scale,
                },
                inputs=inputs + pos.outputs,
                outputs=outputs,
            )
...
with torch.no_grad():
    with ENABLE_CUDA_KERNEL():
        ...
        processor.fuse_selfattention_encoder()
        processor.fuse_gelu()
