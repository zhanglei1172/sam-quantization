...
try:
    ...
    from .utils import matmul248_kernel_config_pruner
    ...
    def matmul_248_kernel(
        ...
        for k in range(0, num_pid_k):
            ...
            b = ((b - zeros) * scales).to(a_ptr.dtype.element_ty)  # Scale and shift
...
def matmul248(input, qweight, scales, qzeros, g_idx, bits, maxq):
    with torch.cuda.device(input.device):
        output = torch.empty(
            (input.shape[0], qweight.shape[1]), device=input.device, dtype=torch.bfloat16
        )
        grid = lambda META: (
...
def transpose_matmul248(input, qweight, scales, qzeros, g_idx, bits, maxq):
    with torch.cuda.device(input.device):
        ...
        output = torch.empty(
            (input.shape[0], output_dim), device=input.device, dtype=torch.bfloat16
        )
        grid = lambda META: (
            triton.cdiv(input.shape[0], META["BLOCK_SIZE_M"])
...
class QuantLinear(nn.Module):
    def __init__(self, bits, groupsize, infeatures, outfeatures, bias):
        ...
        if bias:
            self.register_buffer(
                "bias", torch.zeros((outfeatures), dtype=torch.bfloat16)
            )
        else:
            self.bias = None
    ...
    def pack(self, linear, scales, zeros, g_idx=None):
        ...
        self.scales = scales.clone().to(torch.bfloat16)
