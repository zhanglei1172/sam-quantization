...
def _int8_attention_rel_h_rel_w_kernel_aligned_device(
    inp,
    rel_h,
    rel_w,
    sm_scale,
...
lib.define(
    "custom_flash_aligned(Tensor inp, Tensor rel_h, Tensor rel_w, float sm_scale, int head_num, int hidden_dim) -> Tensor"
)
...
def _attention_rel_h_rel_w_kernel_aligned_meta(
    inp, rel_h, rel_w, sm_scale, head_num, hidden_dim
):
    return inp[..., : head_num * hidden_dim].contiguous()


@torch.library.impl(lib, "int8_custom_flash_aligned", "Meta")
def _int8_attention_rel_h_rel_w_kernel_aligned_meta(
    inp, rel_h, rel_w, sm_scale, head_num, hidden_dim
):
    return inp[..., : head_num * hidden_dim].to(torch.int8).contiguous()


@torch.library.impl(lib, "int8_custom_flash_aligned", "CUDA")
def _int8_attention_rel_h_rel_w_kernel_aligned(
    inp, rel_h, rel_w, qunat_scale_1, sm_scale, head_num, hidden_dim
):
    ...
    key = (*_create_best_configs_key(inp, rel_h, rel_w, o), 1)
    if key not in BEST_CONFIGS:
        ...
        best, best_config = _autotune(
            configs,
            functools.partial(
                _int8_attention_rel_h_rel_w_kernel_aligned_device,
                inp,
                rel_h,
                rel_w,
                sm_scale,
...
def _attention_rel_h_rel_w_kernel_aligned(
    inp, rel_h, rel_w, sm_scale, head_num, hidden_dim
):
    # This is likely not needed, but without it the kernel
    # is guaranteed to fail. If the inputs are already contiguous
...
def _int8_attention_rel_h_rel_w(
    inp, rel_h, rel_w, qunat_scale_1, head_num, hidden_dim, sm_scale
):
    """
    Writing this as a composite allows torch.compile to fuse
    the needed padding into previous operations and memory
    ...
    return torch.ops.customflash.int8_custom_flash_aligned(
        inp, rel_h, rel_w, qunat_scale_1, sm_scale, head_num, hidden_dim
    )
