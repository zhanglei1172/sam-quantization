...
class QAct(nn.Module):
    def __init__(
        ...
        self.observer = build_observer(
            self.observer_str, self.module_type, self.bit_type, self.calibration_mode, permute=permute
        )
        self.quantizer = build_quantizer(
            self.quantizer_str, self.bit_type, self.observer, self.module_type, permute=permute
        )
...
class QIntSoftmax(nn.Module):
    ...
    def int_softmax(x, scaling_factor):
        ...
        def int_exp(x_int, scaling_factor):
            ...
            q = torch.floor(x_int / x0_int)  # q_ln2
            r = x_int - x0_int * q  # q_p
