...
class PtfObserver(BaseObserver):
    def __init__(self, module_type, bit_type, calibration_mode, permute=True):
        super(PtfObserver, self).__init__(
            module_type, bit_type, calibration_mode, permute=permute
        )
    ...
    def get_quantization_params(self, inputs, *args, **kwargs):
        ...
        # size = inputs.shape[2]
        if len(inputs.shape) == 4 and self.permute: # for Conv2d
        ...
        for j in range(size):
            ...
            score1 = lp_loss(data, data_q1, p=2.0, reduction="all").item()
            score2 = lp_loss(data, data_q2, p=2.0, reduction="all").item()
            score4 = lp_loss(data, data_q4, p=2.0, reduction="all").item()
            score8 = lp_loss(data, data_q8, p=2.0, reduction="all").item()
